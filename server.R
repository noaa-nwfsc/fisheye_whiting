...
shinyServer(function(input, output, session) {
  ...
  output$stat3Input <- renderUI({
    radioButtons(
      ...
      selected = "Utilization by weight"
    )
  })
  ...
  filtered_dt <- reactive({
    if (input$tab_type == "Summary") {
      ...
      data_table %>%
        filter(
          Metric %in%
            c('Commercial catch', 'Final allocation', 'Initial allocation'),
          Statistic %in% input$stat3Input,
          Sector %in% input$sector3Input
        )
    }
  ...
  dt_dat <- reactive({
    ...
    dat <- dat %>%
      ...
      rename(!!quo_name(valuetitle) := Value, !!quo_name(vartitle) := Variance)
  ...
  output$tacplot <- renderPlot(
    {
      ...
      ggplot(dat, aes(x = Year, y = Value, group = Metric, fill = Metric)) +
  ...
  output$Email <- renderUI({
    withTags(
      div(
        ...
        p(
          h4("Please email us at", strong("nmfs.nwfsc.fisheye@noaa.gov")),
